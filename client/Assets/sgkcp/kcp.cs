// file from https://github.com/limpo1989/kcp-csharp
using System;
...
namespace SG.Network.skynet
{
    public class KCP
    {
        ...
        public int PeekSize()
        {
            ...
            {
                length += rcv_queue[i].data.Length;
                if (0 == rcv_queue[i].frg)
                    break;
            }
        ...
        public int Recv(byte[] buffer)
        {
            ...
            for (int i = 0; i < rcv_queue.Length; i++)
            {
                Array.Copy(rcv_queue[i].data, 0, buffer, n, rcv_queue[i].data.Length);
                n += rcv_queue[i].data.Length;
                count++;
                if (0 == rcv_queue[i].frg) break;
            }
            ...
            {
                if (rcv_buf[i].sn == rcv_nxt && rcv_queue.Length < rcv_wnd)
                {
                    rcv_queue = append<Segment>(rcv_queue, rcv_buf[i]);
                    rcv_nxt++;
                    count++;
                }
                else
        ...
        void parse_una(uint una)
        {
            ...
            for (int i = 0; i < snd_buf.Length; i++)
            {
                if (_itimediff(una, snd_buf[i].sn) > 0)
                    count++;
                else
                    break;
            }
        ...
        {
            acklist = append<UInt32>(acklist, new uint[2] { sn, ts });
        }
        ...
        public uint Check(uint current_)
        {
            ...
            for (int i = 0; i < snd_buf.Length; i++)
            {
                var diff = _itimediff(snd_buf[i].resendts, current_);
                if (diff <= 0) return current_;
                if (diff < tm_packet) tm_packet = diff;
            }

            minimal = tm_packet;
            if (tm_packet >= tm_flush_) minimal = tm_flush_;
        ...
        public void SetMinrto(uint minrto)
        {
            rx_minrto = minrto;
        }
